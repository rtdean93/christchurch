	...
	*/
	...
	function rtGetSubMenuLink( $mitem, $level, $hilight=false , $color_index=false) {
		...
		switch ($mitem->browserNav) {
			...
			$txt = '<li'. $id . '><a href="'. $mitem->link .'" target="_blank">'. $mitem->name ."</a>";
			break;

			case 2:
			// open in a popup window
			$txt = "<li". $id . "><a href=\"#\" onclick=\"javascript: window.open('". $mitem->link ."', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=780,height=550'); return false\">". $mitem->name ."</a>";
			break;

			case 3:
			// don't link it
			$txt = '<li'. $id . '><span class="seperator">'. $mitem->name ."</span>";
			break;

			default:	// formerly case 2
			// open in parent window
			$txt = '<li'. $id . '><a href="'. $mitem->link .'">'. $mitem->name ."</a>";
			break;
		}
