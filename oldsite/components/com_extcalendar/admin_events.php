...
if(!empty($event_mode)) {
	...
	switch($event_mode) {
		...
		case 'edit':
			if(!empty($extid) && require_priv($CONFIG_EXT['who_can_edit_events'])) {
				$event_mode = 'view';
		...
		case 'view' :
			if(!empty($extid) && require_priv('Administrator'))  {
		...
		case 'del' :
			if( !empty($extid) && require_priv($CONFIG_EXT['who_can_delete_events']) ) {
				$event_mode = 'view';
...
/* print a blank owner form so we can add a new owner */
	...
	global $CONFIG_EXT, $database, $zone_stamp, $errors, $today, $lang_event_admin_data, $lang_general, $lang_settings_data;
...
// If not, we enter a special end_date instead.
					...
					theme_redirect_dialog($lang_event_admin_data['add_event'], $lang_event_admin_data['add_event_success'], $lang_general['continue'], $sef_href);
			...
			// Render the form
      if(!$successful) {
				$sef_href = sefRelToAbs( $CONFIG_EXT['calendar_calling_page'].'&amp;event_mode=add' );
